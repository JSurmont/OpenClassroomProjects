...
def test_signup_route():
    ...
    # Testing if the SignUpView renders the correct template ( registration/signup.html )
    # with correct Form ( SignupForm )
...
def test_change_password_route():
    ...
    response = CLIENT.post(reverse('change-password'), change_password_credentials)
    assert response.status_code == 302

    # To make sure everything went fine, we test again by logging the user with new password.
    response = CLIENT.post(reverse('login'),
                           {'username': 'TestUser', 'password': change_password_credentials['new_password1']})
    assert response.status_code == 302
