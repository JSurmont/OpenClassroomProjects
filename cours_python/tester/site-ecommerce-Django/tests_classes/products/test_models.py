...
class TestProductsModels:
    ...
    def test_is_favourite(self):
        """
        We are creating a Product object, a user object and also a temporary user to pass Login Check,
        ...
        user = User.objects.create(username=credentials['username'], password=credentials['password1'],)

        self.client.post('/user/signup/', credentials)
        self.client.post('/user/login/', {'username': credentials["username"], 'password': credentials["password1"]})

        FavouriteProduct.objects.create(user=user, product=product, is_favourite=True)
