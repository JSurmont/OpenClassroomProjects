...
if NEWS_API_KEY is None:
    # URL de test :
    NEWS_API_URL = "https://s3-eu-west-1.amazonaws.com/course.oc-static.com/courses/4525361/top-headlines.json"
else:
    # URL avec clé :
    NEWS_API_URL = ("https://newsapi.org/v2/top-headlines?sortBy=publishedAt&pageSize=100&language=fr&apiKey="
                    + NEWS_API_KEY)
    ...
    if response.status_code != 200:
        return jsonify({
            ...
            'message': 'La requête à l\'API météo n\'a pas fonctionné.'
                       'Voici le message renvoyé par l\'API : {}'.format(content['message'])
        }), 500

    data = []
    for prev in content["list"]:
        datetime = prev['dt'] * 1000
        temperature = prev['main']['temp'] - 273.15
...
def get_news():
    ...
    if response.status_code != 200:
        return jsonify({
            'status': 'error',
            'message': 'La requête à l\'API des articles d\'actualité n\'a pas fonctionné.'
                       'Voici le message renvoyé par l\'API : {}'.format(content['message'])
        }), 500
